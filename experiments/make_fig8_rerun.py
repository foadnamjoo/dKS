...
ax.set_xlabel("sample size  $n$", fontsize=17)          # LINEAR n (paper convention)
ax.set_ylabel("rejection rate", fontsize=17)
ax.set_title("Power vs $n$", fontsize=18)
...
ax.set_ylabel("runtime per test  (s)", fontsize=17)
ax.set_title("Runtime vs $n$", fontsize=18)
_xaxis_n(ax)
...
ax.set_ylim(-0.02, 1.02)
# shown at 0.7\textwidth vs 0.48 for panels 1-2, so scale title+labels by 0.48/0.7
# to match their ON-PAGE size; ticks and legend are left at full size on purpose.
ax.set_xlabel("runtime per test  (s)", fontsize=12)
ax.set_ylabel("rejection rate", fontsize=12)
ax.set_title("CSR power vs runtime", fontsize=12)
ax.grid(alpha=0.3, which="both")
two_legends(ax)
fig.tight_layout()
