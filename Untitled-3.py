...
def baseline_alg(points1, points2):
    ...
    # Combine, deduplicate, and sort all unique x and y values
    unique_x = np.unique(np.concatenate([points1[:, 0], points2[:, 0]]))
    unique_y = np.unique(np.concatenate([points1[:, 1], points2[:, 1]]))
    ...
    for i in range(len(unique_x) - 1):
        ...
        for j in range(len(unique_y) - 1):
            ...
            x_min, x_max = unique_x[i], unique_x[i + 1]
            y_min, y_max = unique_y[j], unique_y[j + 1]

            # Count points from points1 within the region
            p_count = np.sum((points1[:, 0] >= x_min) & (points1[:, 0] < x_max) &
                             (points1[:, 1] >= y_min) & (points1[:, 1] < y_max))
                        # Count points from points2 within the region
            q_count = np.sum((points2[:, 0] >= x_min) & (points2[:, 0] < x_max) &
                             (points2[:, 1] >= y_min) & (points2[:, 1] < y_max))

            # Update pq_diff based on counts
            pq_diff += (p_count - q_count)

            # Update max_diff if needed
            max_diff = max(max_diff, abs(pq_diff))

    return max_diff / len(points1)

# Generate test data and run the algorithm
result = baseline_alg(p, q)
print("Result:", result)
...
def plot_heatmap_with_all_max_diff(points1, points2, unique_x, unique_y, differences):
    """
    Plots a heatmap of differences between two datasets over a grid defined by unique_x and unique_y.
    
    Parameters:
        points1 (ndarray): Array of points from dataset 1.
        points2 (ndarray): Array of points from dataset 2.
        unique_x (ndarray): Sorted unique x values defining grid boundaries.
        unique_y (ndarray): Sorted unique y values defining grid boundaries.
        differences (list): List of differences calculated for each grid cell.
    """
    # Reshape differences into a 2D array for the heatmap
    grid_differences = np.array(differences).reshape(len(unique_x) - 1, len(unique_y) - 1)
    
    # Create a heatmap plot
    plt.figure(figsize=(8, 6))
    heatmap = plt.imshow(grid_differences.T, origin='lower', cmap='viridis', 
                         extent=[unique_x[0], unique_x[-1], unique_y[0], unique_y[-1]], aspect='auto')
    
    plt.colorbar(heatmap, label='Differences')
    plt.scatter(points1[:, 0], points1[:, 1], color='red', label='Dataset 1', alpha=0.6, s=10)
    plt.scatter(points2[:, 0], points2[:, 1], color='blue', label='Dataset 2', alpha=0.6, s=10)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Heatmap of Differences with Points')
    plt.legend()
    plt.grid(False)
    plt.show()


plot_heatmap_with_all_max_diff(p, q, unique_x, unique_y, differences)


# %%
def plot_grid_and_points_with_axes(points1, points2, unique_x, unique_y):
    ...
    # Set axis limits
    plt.xlim(unique_x[0], unique_x[-1])
    plt.ylim(unique_y[0], unique_y[-1])
...
def plot_heatmap_with_all_max_diff(points1, points2, unique_x, unique_y, differences):
    ...
    # Reshape differences into a grid (row-major order)
    grid = np.array(differences).reshape((num_rows, num_columns))

    # Find all cells with the maximum difference
    max_diff = np.max(grid)
    max_positions = np.argwhere(grid == max_diff)  # Get positions of max value

    # Create the heatmap
    plt.figure(figsize=(10, 8))
    plt.pcolormesh(unique_x, unique_y, grid, cmap='hot', shading='auto')
    plt.colorbar(label='Difference')

    # Highlight cells with the maximum difference
    for row_index, col_index in max_positions:
        x_min, x_max = unique_x[col_index], unique_x[col_index + 1]
        y_min, y_max = unique_y[row_index], unique_y[row_index + 1]
        plt.gca().add_patch(
            plt.Rectangle(
                (x_min, y_min),  # Bottom-left corner
                x_max - x_min,   # Width
                y_max - y_min,   # Height
                edgecolor='magenta', linewidth=3, fill=False,
            )
        )
        # Add label for the maximum difference
        plt.text(
    ...
    plt.xlim(unique_x[0], unique_x[-1])
    plt.ylim(unique_y[0], unique_y[-1])
    plt.title('Heatmap with Points and Maximum Differences Highlighted')
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
    plt.legend()
    plt.grid(False)
    ...
    grid = np.zeros((len(unique_y) - 1, len(unique_x) - 1))  # Correct row-first grid
    index = 0
    for i in range(len(unique_x) - 1):
        for j in range(len(unique_y) - 1):
            grid[j, i] = differences[index]  # Fill grid row-first
            index += 1

    # Find all cells with the maximum difference
    max_diff = np.max(grid)
    max_positions = np.argwhere(grid == max_diff)  # Get all positions of max value

    # Create the figure
    plt.figure(figsize=(10, 8))

    # Draw the heatmap with exact cell boundaries
    plt.pcolormesh(unique_x, unique_y, grid, cmap="hot", shading="flat")
    plt.colorbar(label="Difference")

    # Highlight all cells with the maximum difference
    for row_index, col_index in max_positions:
        x_min, x_max = unique_x[col_index], unique_x[col_index + 1]
        y_min, y_max = unique_y[row_index], unique_y[row_index + 1]
        plt.gca().add_patch(
        ...
        # Add a label for each max difference cell
        plt.text(
            (x_min + x_max) / 2, (y_min + y_max) / 2,  # Center of the rectangle
            f"{max_diff:.2f}",
            color='black', fontsize=8, ha='center', va='center', fontweight='bold'
        )

    # Add the points for `p` and `q`
    plt.scatter(points1[:, 0], points1[:, 1], color='blue', alpha=0.6, label='Set 1 (p)')
    plt.scatter(points2[:, 0], points2[:, 1], color='red', alpha=0.6, label='Set 2 (q)')
    ...
    # Titles and labels
    plt.title("Heatmap with Points and All Max Differences Highlighted")
    plt.xlabel("X-axis")
    plt.ylabel("Y-axis")
    plt.legend()
    plt.grid(False)
...
start_time = time()
unique_x, unique_y, differences, max_diff = baseline_alg(p, q)
runtime = time() - start_time

# Plot the grid and points
plot_grid_and_points_with_axes(p, q, unique_x, unique_y)

# Plot the heatmap with consistent axes
plot_heatmap_with_all_max_diff(p, q, unique_x, unique_y, differences)
...
#from here checked !
 ...
 # Plot the heatmap
    ...
    return Cp, Cq, max_diff, max_diff_position
